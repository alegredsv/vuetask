...
function writeBillsReceived($bills)
{
    $data = ['bills' => $bills];
    $json = json_encode($data);
    file_put_contents(__DIR__ . '/bills-received.json', $json);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function getBillsReceived()
{
    $json = file_get_contents(__DIR__ . '/bills-received.json');
    $data = json_decode($json, true);
    return $data['bills'];
}

$app->get('api/billsReceived', function () use ($app) {
    $bills = getBillsReceived();
    return $app->json($bills);
});


$app->get('api/billsReceived/total', function () use ($app) {
    $bills = getBillsReceived();
    $sum=0;
    foreach ($bills as $value) {
        $sum += (float)$value['value'];
...
$app->get('api/billsReceived/{id}', function ($id) use ($app) {
    $bills = getBillsReceived();
...
$app->post('api/billsReceived', function (Request $request) use ($app) {
    $bills = getBillsReceived();
    $data = $request->request->all();
    $data['id'] = rand(100,100000);
    $bills[] = $data;
    writeBillsReceived($bills);
    return $app->json($data);
});

$app->put('api/billsReceived/{id}', function (Request $request, $id) use ($app) {
...
$app->delete('api/billsReceived/{id}', function ($id) {
    $bills = getBillsReceived();
    $index = findIndexByIdReceive($id);
    array_splice($bills,$index,1);
    writeBillsReceived($bills);
